                ...
                <tr id="curso_<?=$n_cantidad?>" class="<?=$tr_panel?>">
                    ...
                    <?php   if(in_array('ROLE_TESORERIA',$CI->session->userdata('roles'))): ?>
                            ...
                            <td style='padding:5px; border-right:1px solid #9FC5CC;  text-align:center'>
                                ...
                                <?php  if (isset($curso->N_IMPORTE_PROFESOR)):  ?>
...
if(!function_exists('enviar_email'))
{
    ...
    function enviar_email( $email_to , $asunto, $mensaje )
    {
        ...
        $CI->email->from("tesoreria@ucema.edu.ar");

        $CI->email->subject('SITECO - cursos sin confirmar');

        $mensaje_email = '';
        $texto = html_entity_decode( $mensaje , ENT_QUOTES, "UTF-8");
...
if(!function_exists('generar_json_editar_coordinador'))
{
    ...
    function generar_json_editar_coordinador($id_materia, $materia, $id_profesor, $nombre, $id_curso, $anio_lectivo, $puntos, $id_horario, $rol , $sueldo, $c_identificacion, $c_programa, $c_orientacion, $n_cantidad)
    {
        ...
        //var_dump($row_datos_coordinador);
...
if(!function_exists('generar_json_enviar_email_cursos_sin_confirmar'))
{
    function generar_json_enviar_email_cursos_sin_confirmar($row)
    {
        $datos["id_profesor"] = $row->N_ID_PERSONA;
        $datos["id_materia"] = $row->N_ID_MATERIA;
        $datos["id_coordinador"] = $row->ID_PERSONA_COORDINADOR;
        $datos["c_indentificacion"] = $row->C_IDENTIFICACION;
        $datos["c_programa"] = $row->C_PROGRAMA;
        $datos["c_orientacion"] = $row->C_ORIENTACION;


        $datos_enviar = json_encode($datos);
        $datos_enviar = str_replace("\"", "&", $datos_enviar); 
        return $datos_enviar;
    }
}
...
if(!function_exists('generar_json_profesor_factura'))
{

    function generar_json_profesor_factura($id_profesor, $id_curso, $anio_lectivo, $id_horario, $fecha_factura, $rol, $numero_factura)
    {
        $datos_row =  array();
        ...
        $datos_row['anio_lectivo'] = $anio_lectivo;
        $datos_row['id_horario'] = $id_horario;
        $datos_row['fecha_factura'] =  str_replace("-", "&", $fecha_factura); 
        $datos_row['rol'] = urldecode($rol);
        $datos_row['numero_factura'] = $numero_factura;
