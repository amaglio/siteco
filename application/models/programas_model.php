...
public function cursos_asignados_programa_ejepac($c_identificacion,$c_programa,$c_orientacion,$anio)
{
	...
	$sql = 	utf8_decode("   SELECT min(cl.f_clase) as fecha_clase, 
							...
							FROM 
								...
								LEFT JOIN clases cl ON ( 	res.n_curso = cl.n_curso
															AND res.n_id_persona = cl.n_id_persona
															AND res.anio_lectivo = cl.C_AÑO_LECTIVO
															AND res.c_tipo_clase = cl.c_tipo_clase
														)
							WHERE 
							cl.F_CANCELACION is  NULL
							GROUP BY 
							       res.anio_lectivo, res.n_curso, res.n_id_materia, res.f_inicio,res.f_fin, res.d_descrip,res.Horario, res.c_tipo_periodo, res.n_periodo,
							       res.n_id_horario, res.n_id_persona, res.c_tipo_clase, res.c_rol, res.importe, res.n_porcentaje, res.f_pagado,res.C_CONFIRMADO,res.C_OBS,res.N_IMPORTE_PROFESOR,
							       res.materia,
							       ...
							       res.anio,
							       res.comision, res.c_identificacion, res.c_programa,res.c_orientacion
							ORDER BY n_curso, min(cl.f_clase) ASC  ") ;
...
public function desconfirmar_curso($data)
{
		...
		// Antes de desconfirmar me fijo si TESORERIA ya le puso el IMPORTE

		
		$sql_tesoreria = utf8_decode("	SELECT * 
										FROM CURSOS_PROFESORES cp , cursos c, Personas p
										WHERE 	cp.N_CURSO = ? AND  
												cp.C_AÑO_LECTIVO = ?	AND
												cp.C_ROL = ?	AND
												cp.N_ID_PERSONA = ? AND
												cp.N_ID_HORARIO = ? AND
												cp.IMPORTE IS NOT NULL AND
												cp.n_curso = c.n_curso AND 
												cp.n_id_persona = p.n_id_persona AND
												c.C_AÑO_LECTIVO = cp.C_AÑO_LECTIVO")	;

		$query_tesoreria = $this->db->query($sql_tesoreria, array( $data['id_curso'], $data['anio_lectivo'], $data['rol'], $data['idUsuario'], $data['id_horario'] ));
		

 
		if( $query_tesoreria->num_rows() > 0): // Esto significa que TESORERIA YA LE HABIA PUESTO UN IMPORTE
			...
			$this->email->from("ammagliola@ucema.edu.ar");
			$this->email->subject('SITECO - Se desconfirmo un curso con importe');

			$mensaje_email = 'Se desconfirmo un curso que tenia IMPORTE DE TESORERIA: '.$query_tesoreria->row()->D_DESCRIP."<br>";
			$mensaje_email .= "Profesor: ".$query_tesoreria->row()->D_APELLIDOS.", ".$query_tesoreria->row()->D_NOMBRES;
			$texto = html_entity_decode( $mensaje_email , ENT_QUOTES, "UTF-8");
        	$this->email->message($texto);
        	
        	$this->email->to("megarcia@ucema.edu.ar"); 

        	if( $this->email->send() ):

				chrome_log("Envio a email "); 
			
			else:
				
				chrome_log("No Envio a email "); 
...
{	
	...
	/*
	echo "	SELECT devuelve_programa( co.c_identificacion, co.c_programa, co.c_orientacion) as Programa, c.n_id_materia, c.d_descrip
	...
	*/
	...
	$query = $this->db->query( $sql, array( $anio, $anio, $id_materia ) );
...
{	
	...
	//echo utf8_encode(" SELECT importe_profesor( $rol, $tipo_clase, $c_identificacion , $n_id_materia, $c_programa, $c_orientacion, $n_id_persona , $anio_programa, $id_curso, $id_horario) as IMPORTE
	//			    	 FROM  dual "   	);	 
...
{	
    /*echo " SELECT clases_PROFESOR( $c_identificacion, $id_curso , $n_id_persona , $anio_lectivo)as CLASES
				    	 FROM  dual " ;*/
...
public function cursos_cambiados() 	
{
	...
	return $this->db->query(utf8_decode(" 	SELECT cpc.*
...
/*
	Trae los cursos que estna proximos a comenzar
*/
public function cursos_sin_confirmar_enviar_email()	
{
	$dia = '01';
	$mes = date('m')+2;
	$anio = date('Y');
	...
	return $this->db->query(utf8_decode("  	SELECT 
											...
											WHERE 
											    ...
											    cu.f_inicio <=  to_date('$fecha','dd/mm/yyyy') AND
	...
	return $this->db->query(utf8_decode(" SELECT cu.C_AÑO_LECTIVO as anio_lectivo,
							...
							WHERE 
							    ...
							    -- TRAER UN SOLO CURSO PROFESOR NO DOS SI HAY UNO CON MAS DE UN HORARIO			
                 				...
                 				( cp.importe is null or cp.importe = '' ) AND	
...
public function cantidad_cursos_modificados()
{
	chrome_log("model/cantidad_cursos_modificados" );

	$sql = 	utf8_decode("	SELECT *
							FROM cursos_profesores_cambios cpc
							WHERE
							     cpc.visto = 0  ") ;
...
public function cambiar_cambios_a_vistos()
{
	chrome_log("model/cambiar_cambios_a_vistos" );

	$sql = 	utf8_decode("	UPDATE cursos_profesores_cambios cpc
							SET   cpc.visto = 1 
							WHERE cpc.visto = 0  ") ;
...
{
	chrome_log("profesor/get_materia_nombre" );

	$sql = 	utf8_decode("	SELECT  D_DESCRIP 
							FROM    materias
							WHERE n_id_materia = ?  " ) ;

	$query = $this->db->query($sql, array( $id_materia ) );

	return $query->row()->D_DESCRIP;
}
